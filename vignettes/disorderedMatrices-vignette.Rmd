---
title: "disorderedMatrices-vignette"
author: "William McFadden"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{disorderedMatrices-vignette}
...
The following amino acid substitution matrices derived from sets of IDPs and
...
Downloading and attaching idpr
```{r}
...
accelerated evolution, and has been observed (
...
IDPs typically have a conservation of chemistry and disordered state, but less 
so at the residue and position position level. Therefore the
use of BLOSUM and PAM for studies comparing IDP evolution are not as appropriate
since these matrices account for evolutionary observations of ordered proteins(
[Brown et al. 2011](https://doi.org/10.1016/j.sbi.2011.02.005)).
...
accurately analyze the evolution of IDPs. Therefore, the works of 
[Trivedi & Nagarajaram 2019](https://doi.org/10.1038/s41598-019-52532-8),
...
as references for IDP-based substitution matrices. These each use experimentally
...
within R packages allow for custom substitution matrices. **idpr** has added 11 
disorder-based substitution matrices for IDP sequence conservation studies for
user convenience. 
...
depending on how disordered the sequences were. Protein families were sorted 
...
### EDSS Matrices
EDSSMat62, like the other EDSS matrices, are derived from alignment blocks of
computationally predicted IDPs. 
[Trivedi & Nagarajaram 2019](https://doi.org/10.1038/s41598-019-52532-8)  
go into detail of methods and comparisons. EDSSMat62 the best preforming matrix
in their study and was shown to identify both close and distant homologs of 
Secretogranin-1, an IDP, while other matrices could only identify some 
...
EDSS Matrices are symmetric, 24x24 matrices. It has the 20 standard amino acids
...
These matrices are from 
[Brown et al. 2009](https://doi.org/10.1093/molbev/msp277),
found protein families based on experimentally confirmed IDPs.
These were not designed with the goal of improving sequence alignments.
The authors note that the purpose of these matrices were to compare evolutionary
characteristics of disordered and ordered proteins.
Therefore information for using these in sequence alignments, like gap scores
and entropy, are not provided by the authors.  
[Trivedi & Nagarajaram 2019](https://doi.org/10.1038/s41598-019-52532-8)
determined the optimal gap parameters for these matrices to compare against 
EDSSMat62. 

Disorder40, Disorder60, and Disorder85 are symmetric, 24x24 matrices. It has the 20 standard amino acids
and 4 ambiguous residues: B, Asparagine or Aspartic Acid (Asx);
Z, Glutamine or Glutamic Acid (Glx); X, Unspecified or unknown amino acid; and 
*, Stop.
```{r}
...
This was an early attempt at improving alignments for IDPs. 
...
[The UniProt Consortium 2018](https://doi.org/10.1093/nar/gky1049)).
```{r}
TP53_Sequences <- idpr:::TP53Sequences
...
The **msa** vignette shows an example of using the multiple sequence alignment
(generated above) into a phylogenetic tree of sequence distances.
This uses both the **ape** and **seqinr** packages. 

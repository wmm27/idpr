...
The functions iupred(), iupredAnchor(), and iupredRedox() are all functions 
...
One example of these is IUPred2A. 
...
IDPs lack of secondary and tertiary structure (Dosztányi, 2018). 
...
arguments. To make this connection requires that the user to know the UniProt
...
This prediction is similar to relevant disordered segments curated within 
the DisProt Database (Dosztányi, 2018; Hatos et al., 2019). 
...
that are likely to have a specific and rigid fold (Dosztányi, 2018). 
```{r}
p53_ID <- "P04637"
iupred(p53_ID,
       proteinName = "HUMAN P53",
...
The data frame has an additional column for ANCHOR2 scores
```{r}
...
## iupredRedox
Another element influencing the environmental chemistry is the redox potential. 
...
oxidizing (minus) environment. The prediction is done by modifying all 
cystine residues to serine when simulating a “redox-minus” environment.
...
(Normal prediction) and a redox-minus environment (Cys --> Ser). 

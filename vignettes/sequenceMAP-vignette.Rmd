---
title: "sequenceMAP-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{sequenceMAP-vignette}
...
the entire sequence and color it based on properties. This may help identify 
important residues within a protein.
...
To make a sequence map you will need two vectors of data (typically as a 
data frame). One vector is the amino acid sequence and another is a vector
of data to color. 
...
The output of structuralTendency() gives us both the amino acid sequence in the
column 'Residue' and the values in 'Tendency'

The output of chargeCalculationLocal(plotResults = FALSE) 
gives us both the amino acid sequence in the column 'aa' and the values
in 'Charge'
```{r}
sequenceMap(
...
The text can also be rotated for ease of reading, espeically helpful for 
larger sequences
```{r}
sequenceMap(
  sequence = tendencyDF$AA,
...
sequenceMap(
  ...
  labelType = "number",
  labelLocation = "on",
  everyN = 30) #Every 30th residue is printed
```

You can also change the number of residues on each line with nbResidues. 
This helps improve visualization based on the sequence
```{r}
sequenceMap(
  sequence = tendencyDF$AA,
  property = tendencyDF$Tendency,
  labelType = "number",
  labelLocation = "on",
  nbResidues = 30) #30 residues each row
...
You can also specify colors for discrete values using a vector of colors
```{r}
sequenceMap(
  sequence = tendencyDF$AA,
  property = tendencyDF$Tendency,
...
Since the output is a ggplot, the visualization is able to be stored and 
additional features can be added / annotated
```{r}
library(ggplot2)
...
To make annotations easy, the sequenceMapCoordinates() will give you 
the row (y value) and the colun (x value) of each residue visualized with 
sequenceMap(). 
...
#Change / add the horizontal lines
sequencePlot(
position = exampleDF$Position,
property = exampleDF$Charge,
hline = 0.0,

---
title: "chargeHydropathy-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{chargeHydropathy-vignette}
...
```{r}
HUMAN_P53 <- idpr:::TP53Sequences[2]
print(HUMAN_P53)
```
...
Since a ggplot is returned, the cooridnates will be annotated
...
labels, custom themes, or additional features
```{r}
gg <- gg + ggplot2::theme_void() + #Add a different theme
            ggplot2::annotate("text", #Add text annotations
                    x = P53_msh - 0.05,
                    y = P53_nc,
                    label = "TP53\nSequences")
plot(gg)
...
Creating a data frame of window hydropathy scores
```{r}
...
Plotting each window hydropathy scores along the sequence
```{r}
...
The window size can be specified as well. This must be an odd integer
...
hendersonHasselbalch function). Therefore the pH and pKa are important for 
calculations.
...
There are also many pKa sets that are preloaded. 
...
Additionally, see Kozlowski (2016) for additional details on pKa data sets. 
...
Here is an example using the pKa values from 
...
Creating a data frame of charge for each residue
```{r}
